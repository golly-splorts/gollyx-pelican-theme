# gollyx-vii-pelican-theme

Pelican theme for the the Hellmouth VII Cup website.

Basically a clone of <https://github.com/golly-splorts/gollyx-v-pelican-theme> with some minor tweaks.

The Pelican theme consists of all elements of the UI that are common to
all pages of the Golly UI.

## Installation

To install the theme:

```
git clone <this repo>

# If theme is not installed, install it
pelican-themes -i gollyx-vii-pelican-theme

# If theme is installed, update it
pelican-themes -U gollyx-vii-pelican-theme
```

## Usage

To use this Pelican theme, set the theme to `gollyx-vii-pelican-theme`
in `pelican.conf`.

```
THEME = 'gollyx-vii-pelican-theme'
```
